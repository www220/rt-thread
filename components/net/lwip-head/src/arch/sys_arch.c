...
while(netif != RT_NULL)
{
    ...
    if (!(ethif->flags & NETIF_FLAG_DHCP) && (ethif->flags & NETIF_FLAG_ETHARP))
#else
    int addrindex = 0;
...
#endif
    {
        ...
        if (NET_ADDR[addrindex][0][0])
            ip4_addr_set_u32(&ipaddr, ipaddr_addr(NET_ADDR[addrindex][0]));
        else
            IP4_ADDR(&ipaddr, RT_LWIP_IPADDR0, RT_LWIP_IPADDR1, RT_LWIP_IPADDR2, RT_LWIP_IPADDR3);
        if (NET_ADDR[addrindex][2][0])
            ip4_addr_set_u32(&gw, ipaddr_addr(NET_ADDR[addrindex][2]));
        else
            IP4_ADDR(&gw, RT_LWIP_GWADDR0, RT_LWIP_GWADDR1, RT_LWIP_GWADDR2, RT_LWIP_GWADDR3);
        if (NET_ADDR[addrindex][1][0])
            ip4_addr_set_u32(&netmask, ipaddr_addr(NET_ADDR[addrindex][1]));
        else
