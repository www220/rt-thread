...
#define RTTHREAD_MUTEX_INITIALIZER { { { {0} } } }

#if SQLITE_MUTEX_NREF
#define SQLITE3_MUTEX_INITIALIZER { RTHREAD_MUTEX_INITIALIZER, 0, 0, (pthread_t)0, 0 }
#else
#define SQLITE3_MUTEX_INITIALIZER { RTTHREAD_MUTEX_INITIALIZER }
#endif
...
static sqlite3_mutex *pthreadMutexAlloc(int iType){
  ...
  switch( iType ){
    ...
    default: {
      ...
      p->id = iType;
#endif
      if (p->mutex.parent.parent.name[0] == 0) {
        sprintf(name, "sqlmtx%d", iType);
        rt_mutex_init(&p->mutex, name, RT_IPC_FLAG_FIFO);
      }
      break;
