...
struct mxs_platform_fb_entry {
	...
	u32 dclk_f;
	int lcd_type;
	int (*init_panel) (struct rt_device *, dma_addr_t, int,
			   struct mxs_platform_fb_entry *);
				      ...
				      u16 h_active, int enable_present)
{
	...
	writel(BF_LCDIF_CTRL1_BYTE_PACKING_FORMAT(0xf) |//7
	...
	writel(BM_LCDIF_CTRL_WORD_LENGTH |
		     ...
		     REGS_LCDIF_BASE + HW_LCDIF_CTRL_CLR);
	writel(BF_LCDIF_CTRL_WORD_LENGTH(0) |	/* 0:16bit 3:24 bit */
...
static inline void release_dotclk_panel(void)
{
	writel(BM_LCDIF_CTRL_DOTCLK_MODE,
		     REGS_LCDIF_BASE + HW_LCDIF_CTRL_CLR);
	writel(0, REGS_LCDIF_BASE + HW_LCDIF_VDCTRL0);
	writel(0, REGS_LCDIF_BASE + HW_LCDIF_VDCTRL1);
	writel(0, REGS_LCDIF_BASE + HW_LCDIF_VDCTRL2);
	writel(0, REGS_LCDIF_BASE + HW_LCDIF_VDCTRL3);
}
