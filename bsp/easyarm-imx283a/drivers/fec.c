...
static void phy_monitor_thread_entry(void *parameter)
{
    ...
    while(1)
    {
        ...
        /* linkchange */
        if(phy_speed_new != phy_speed && reset_count <= 0)
        {
        ...
        else if (phy_speed_new != phy_speed)
        {
            reset_count--;
        }
        else
...
void rt_hw_eth_init(void)
{
	...
	pin_gpio_set(PIN_NET0, 0);
	udelay(200);udelay(200);udelay(200);
	pin_gpio_set(PIN_NET0, 1);
    ...
    eth_device_init_with_flag(&(stm32_eth_device.parent), "e0", flags);
