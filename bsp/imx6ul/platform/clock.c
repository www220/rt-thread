...
void mxs_set_lcdclk(uint32_t base_addr, uint32_t freq)
{
	...
	if (temp < min) {
		...
		if (3 == i) {
			rt_kprintf("Fail to set rate to %dkhz", freq);
			return;
		}
	}
...
unsigned int mxc_get_clock(enum mxc_clock clk)
{
	...
	case MXC_SATA_CLK:
		return get_ahb_clk();
	default:
		rt_kprintf("Unsupported MXC CLK: %d\n", clk);
		break;
	}
...
int do_mx6_showclocks(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	u32 freq;
	freq = decode_pll(PLL_SYS, MXC_HCLK);
	rt_kprintf("PLL_SYS    %8d MHz\n", freq / 1000000);
	freq = decode_pll(PLL_BUS, MXC_HCLK);
	rt_kprintf("PLL_BUS    %8d MHz\n", freq / 1000000);
	freq = decode_pll(PLL_USBOTG, MXC_HCLK);
	rt_kprintf("PLL_OTG    %8d MHz\n", freq / 1000000);
	freq = decode_pll(PLL_ENET, MXC_HCLK);
	rt_kprintf("PLL_NET    %8d MHz\n", freq / 1000000);

	rt_kprintf("\n");
	rt_kprintf("IPG        %8d kHz\n", mxc_get_clock(MXC_IPG_CLK) / 1000);
	rt_kprintf("UART       %8d kHz\n", mxc_get_clock(MXC_UART_CLK) / 1000);
#ifdef CONFIG_MXC_SPI
	rt_kprintf("CSPI       %8d kHz\n", mxc_get_clock(MXC_CSPI_CLK) / 1000);
#endif
	rt_kprintf("AHB        %8d kHz\n", mxc_get_clock(MXC_AHB_CLK) / 1000);
	rt_kprintf("AXI        %8d kHz\n", mxc_get_clock(MXC_AXI_CLK) / 1000);
	rt_kprintf("DDR        %8d kHz\n", mxc_get_clock(MXC_DDR_CLK) / 1000);
	rt_kprintf("USDHC1     %8d kHz\n", mxc_get_clock(MXC_ESDHC_CLK) / 1000);
	rt_kprintf("USDHC2     %8d kHz\n", mxc_get_clock(MXC_ESDHC2_CLK) / 1000);
	rt_kprintf("USDHC3     %8d kHz\n", mxc_get_clock(MXC_ESDHC3_CLK) / 1000);
	rt_kprintf("USDHC4     %8d kHz\n", mxc_get_clock(MXC_ESDHC4_CLK) / 1000);
	rt_kprintf("EMI SLOW   %8d kHz\n", mxc_get_clock(MXC_EMI_SLOW_CLK) / 1000);
	rt_kprintf("IPG PERCLK %8d kHz\n", mxc_get_clock(MXC_IPG_PERCLK) / 1000);
...
{
	do_mx6_showclocks(0,0,0,0);
}
FINSH_FUNCTION_EXPORT(list_clocks, display clocks);
MSH_CMD_EXPORT(list_clocks, display clocks);
#endif
