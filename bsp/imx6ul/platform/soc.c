...
int check_ldo_bypass(void)
{
	...
	node = fdt_node_offset_by_compatible(gd->fdt_blob, -1,
		"fsl,imx6q-gpc");
	if (node < 0) {
		rt_kprintf("No gpc device node %d, force to ldo-enable.\n", node);
		return 0;
	}
...
int check_1_2G(void)
{
	...
	if (((reg >> 16) & 0x3) == 0x3) {
		if (ldo_bypass) {
			rt_kprintf("Wrong dtb file used! i.MX6Q@1.2Ghz only "
			 ...
			 */
			rt_kprintf("Please use imx6q-sabresd-ldo.dtb!\n");
			hang();
		}
		result = 1;
	}
...
int recovery_check_and_clean_flag(void)
{
    ...
    rt_kprintf("check_and_clean: reg %x, flag_set %d\n", reg, flag_set);
