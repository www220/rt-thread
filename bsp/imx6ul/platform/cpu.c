...
int print_cpuinfo(void)
{
	...
	if (is_mx6dqp()) {
		rt_kprintf("CPU:   Freescale i.MX%sP rev%d.%d at %d MHz\n",
...
#endif
	{
		rt_kprintf("CPU:   Freescale i.MX%s rev%d.%d at %d MHz\n",
			get_imx_type((cpurev & 0xFF000) >> 12),
			(cpurev & 0x000F0) >> 4,
			(cpurev & 0x0000F) >> 0,
			mxc_get_clock(MXC_ARM_CLK) / 1000000);
	...
	if (!ret) {
		...
		if (!ret)
			...
			rt_kprintf("CPU:   Temperature: invalid sensor data\n");
	} else {
		rt_kprintf("CPU:   Temperature: Can't find sensor device\n");
	}
#endif

#if defined(CONFIG_DBG_MONITOR)
	if (readl(&dbg->snvs_addr))
		rt_kprintf("DBG snvs regs addr 0x%x, data 0x%x, info 0x%x\n",
		       readl(&dbg->snvs_addr),
		       readl(&dbg->snvs_data),
		       readl(&dbg->snvs_info));
#endif

	rt_kprintf("Reset cause: %s\n", get_reset_cause());
	return 0;
}
#endif
...
void set_chipselect_size(int const cs_size)
{
	...
	case CS0_32M_CS1_32M_CS2_32M_CS3_32M:
		...
		break;
	default:
		rt_kprintf("Unknown chip select size: %d\n", cs_size);
		break;
	}
